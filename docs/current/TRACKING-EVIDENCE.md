...
| `vit-pose-bias-extension` | Read-only bias export and unmodified rebuild produce identical standard estimator output. Relinking the same objects with mold 2.40.4 made both match all 2,553 installed-release rows on the original failing replay. | GNU ld artifacts still diverge. One matched offline replay does not qualify the mold-linked binaries for installation or physical tracking; the original deployment bundle remains rejected. |
...
- [Basalt for Monado](https://gitlab.freedesktop.org/mateosss/basalt) — the backend project. The local release comparison used source commit `30ece25f4c7d86e6a9dbee7ff0ebd0b921344a67` with pinned dependencies. A mold 2.40.4 relink reproduced the release's output on the original failing replay; a clean-build and deployment qualification remain open.
